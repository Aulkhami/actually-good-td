...
--[=[
	...
	@within Spring
	A clock function that is used to update the spring.
]=]
...
export type Spring<T> = {
	...
	new: (initial: T, clock: ClockFunction?) -> Spring<T>,
	Impulse: (self: Spring<T>, velocity: T) -> (),
	TimeSkip: (self: Spring<T>, deltaTime: number) -> (),
}
...
function Spring.new<T>(initial: T, clock: ClockFunction?): Spring<T>
...
function Spring:TimeSkip(deltaTime: number)
	...
	local position, velocity = self:_positionVelocity(now + deltaTime)
